...
void VRE::VRE_GameObjRenderSystem::Render()
{
    ...
    for (auto& e : *mSharedContext->mGameObjMap) {
        ...
        vkCmdPushConstants(mSharedContext->mRenderer->GetCurrentCommandBuffer(), mPipelineLayout,
                           VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(GameObjectBufferData), &data);
...
void VRE::VRE_GameObjRenderSystem::RenderNode(std::shared_ptr<glTFNode> node)
{
    ...
    for (auto& prim : node->mMesh->mPrimitives) {
        ...
        auto imageInfo = prim->mMaterial.mBaseColorTexture->GetDescImageInfo();

        VRE_DescriptorWriter(*mDescSetLayout, *mDescPool)
                             .WriteBuffer(0, &info)
                             .WriteImage(1, &imageInfo)
                             .Build(descSet);

        vkCmdBindDescriptorSets(mSharedContext->mRenderer->GetCurrentCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, mPipelineLayout, 1, 1, &descSet, 0, nullptr);
...
void VRE::VRE_GameObjRenderSystem::CreateDescPools()
{
    ...
    for (auto& e : *mSharedContext->mGameObjMap) {
        for (auto& node : e.second.mModel->GetAllNodes()) {
            meshCount++;
            textureCount += node->mMesh->mPrimitives.size();
        }
    }

    mDescPool = VRE_DescriptorPool::Builder(*mSharedContext->mDevice)
                .SetMaxSets(10000 * VRE_SwapChain::MAX_FRAMES_IN_FLIGHT)
                .AddPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 10000 * VRE_SwapChain::MAX_FRAMES_IN_FLIGHT)
                .AddPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 10000 * VRE_SwapChain::MAX_FRAMES_IN_FLIGHT)
                .Build();
}
