...
VkCommandBuffer VRE::VRE_Renderer::BeginDraw()
{
    ...
    const VkResult& result = mSwapChain->AcquireNextImage(&mCurrentImageIndex);
...
void VRE::VRE_Renderer::EndDraw()
{
    ...
    const VkResult& result = mSwapChain->SubmitCommandBuffers(&commandBuffer, &mCurrentImageIndex);
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || mWindow.HasWindowResized())
    {
...
void VRE::VRE_Renderer::RecreateSwapChain()
{
    ...
    { 
        //Pause if the window is minimized as it can cause swapchain to become out of date.
