...
// TODO: Limit frame rate when using mailbox.
...
void VRE::VRE_App::Run()
{
    ...
    while (!mWindow.ShouldClose())
    {
        ...
        if (VkCommandBuffer commandBuffer = mRenderer.BeginDraw())
        {
...
void VRE::VRE_App::Init()
{
    ...
    VRE_DescriptorPool::Builder objPoolBuilder = VRE_DescriptorPool::Builder(mDevice)
                                                 .SetMaxSets(1000)
    ...
    for (int i = 0; i < VRE_SwapChain::MAX_FRAMES_IN_FLIGHT; i++)
    {
        VkDescriptorBufferInfo bufferInfo = mSceneUBOs[i]->DescriptorInfo();
...
void VRE::VRE_App::LoadObjects()
{
    ...
    {
        VRE_PointLight pointLight = VRE_PointLight::CreatePointLight(1.0f);
        pointLight.mColor = coloredLights[i];
        glm::mat<4, 4, float> rotateLight = glm::rotate(glm::mat4(1.f), (i * glm::two_pi<float>()) / coloredLights.size(), {0.f, 1.f, 0.f});
