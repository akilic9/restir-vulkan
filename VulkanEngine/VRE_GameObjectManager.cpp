...
{
    for (std::pair<const unsigned, VRE_GameObject>& gameObject : mGameObjectsMap)
    {
        VRE_GameObject& obj = gameObject.second;
        GameObjectBufferData data{};
        data.mModelMatrix = obj.mTransform.Mat4();
        data.mNormalMatrix = obj.mTransform.NormalMatrix();
        mUboBuffers[mSharedContext->mRenderer->GetFrameIndex()]->WriteToIndex(&data, gameObject.first);
    }
